...
@Repository
public class TrackingDaoImpl extends GenericDao<Tracking> {

    private static final Logger log =
            LoggerFactory.getLogger(TrackingDaoImpl.class);

    public TrackingDaoImpl(SessionFactory sessionFactory) {
        super(sessionFactory, Tracking.class);
    }

    public List<Tracking> findByShipmentId(Long shipmentId) {

        log.debug(
                "DAO: Finding tracking events for shipment id: {}",
                shipmentId);

        return sessionFactory.getCurrentSession()
                .createQuery(
                        "FROM Tracking t WHERE t.shipmentId = :shipmentId " +
